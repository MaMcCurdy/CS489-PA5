...
function Enemy:init()
    ...
    self.text = {damage = 0, pos = 0, alpha = 1}
    self.tween = Tween.new(3, self.text, {pos = 10, alpha = 0})
end
...
function Enemy:draw()
    ...
    love.graphics.setColor(1, 0, 0, self.text.alpha)
    love.graphics.print("-"..self.text.damage, damageFont, self.x+(self:getHurtbox().width/2), self.y-self.text.pos)
