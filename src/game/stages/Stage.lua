...
function Stage:init(rows, cols, ts)
    ...
    self.progress = 0
    self.complete = false
    self.death = love.graphics.newParticleSystem(love.graphics.newImage("graphics/particles/41.png"), 30)
    self.death:setParticleLifetime(0.2, 1.0)
    self.death:setSizes(0.1, 0)
    self.death:setSpeed(0, 20) 
    self.death:setLinearAcceleration(0, 0, 0, 0)
    self.death:setEmissionArea("uniform",20,20,0,true)
    self.death:setColors(1, 1, 1, 1, 0, 0, 0, 0)
end
...
function Stage:update(dt)
    ...
    for k=#self.mobs, 1, -1 do 
        ...
        if self.mobs[k].died then
            self.death:setPosition(self.mobs[k].x, self.mobs[k].y)
            self.death:emit(30)
    ...
    if self.progress >= self.gems then
        self.complete = true
        gameState = "cleared"
    end
end

function Stage:setGems(g)
    self.gems = g
end

function Stage:addGem(g)
